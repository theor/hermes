...
<form action="/setDebugMode">
    <input type="checkbox" name="input1" value="1">
    <label for="input1">Debug mode</label><br>
    <input type="submit" value="Submit">
</form>
...
void setupServer()
{
    ...
    server.on("/set", HTTP_GET, [](AsyncWebServerRequest *request)
    {
        ...
        if (request->hasParam(PARAM_INPUT_1))
        {
            ...
            request->send(200, "text/html", "Set msg to '" + inputMessage +
                                            "'<br><a href=\"/\">Return to Home Page</a><br><a href=\"/run\">Leave upload mode</a>");
        }
        else
            request->send(200, "text/html", "Missing data.<br><a href=\"/\">Return to Home Page</a>");
    });
