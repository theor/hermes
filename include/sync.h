...
int sync_save_tracks(const struct sync_cb *cb, const struct sync_device *);
#endif /* defined(SYNC_PLAYER) */

struct sync_io_cb {
	void *(*open)(const char *filename, const char *mode);
	size_t (*read)(void *ptr, size_t size, size_t nitems, void *stream);
	size_t (*write)(const void *ptr , const size_t size, const size_t nitems, void *stream);
...
int sync_get_val_int(const struct sync_track *, double);
